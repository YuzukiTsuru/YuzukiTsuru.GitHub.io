...
flowchart TD
    ...
    subgraph IH["偏移 0x0000"]
        A1["Image Header (1024 bytes)"]
        A2["Magic: IMAGEWTY (8 bytes)"]
        A3["Header Version: 0x0100 / 0x0300"]
        A4["Header Size / RAM Base / Version"]
        A5["Image Size / Image Header Size"]
        A6["Version-specific data (V1 / V3)"]
        A7["PID / VID / Hardware ID / Firmware ID"]
        A8["Num Files (文件数量)"]
    ...
    subgraph FH0["偏移 0x0400 (1024)"]
        B1["File Header 0 (1024 bytes)"]
        B2["Filename Length / Total Header Size"]
        B3["Main Type: FES / 12345678 / COMMON"]
        B4["Sub Type: FES_1-0000000000 / ..."]
        B5["Version-specific data (V1 / V3)"]
        B6["Stored Length / Original Length"]
        B7["Offset (数据偏移)"]
        B8["Filename"]
    ...
    subgraph FH1["偏移 0x0800 (2048)"]
        C1["File Header 1 (1024 bytes)"]
    end

    subgraph FHN["..."]
        D1["File Header N (1024 bytes)"]
    end

    subgraph FD["偏移 = File Header N 的 Offset 字段"]
        E1["File Data (实际内容)"]
    end
