---
layout: post
title: "TinyVision 使用 AWOL Tina Linux 支持 OpenCV 开启摄像头"
...
while True:
    ...
    ret, frame = cap.read()
    if not ret:
        print("Failed to read video device.")
        break
    ...
    # 定义帧缓冲区参数
    framebuffer_width = frame_width
    framebuffer_depth = 32
    frame_size = (frame_width, frame_height)
    
    # 将图像转换为4通道BGR格式
    split_bgr = cv2.split(frame)
    split_bgr.append(np.ones((frame_height, frame_width), dtype=np.uint8) * 255)
    framebuffer_compat = cv2.merge(split_bgr)
    
    # 将图像写入帧缓冲区
    if framebuffer_depth == 16:
        framebuffer_compat = cv2.cvtColor(framebuffer_compat, cv2.COLOR_BGR2BGR565)
        fb_file.write(framebuffer_compat.tobytes())
