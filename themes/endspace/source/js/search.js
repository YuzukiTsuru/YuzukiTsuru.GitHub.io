...
(function () {
    ...
    var html = items
      .map(function (p) {
        ...
        if (term) {
          ...
          summary =
            (start > 0 ? '… ' : '') + src.slice(start, end) + (end < src.length ? ' …' : '');
        } else {
        ...
        return (
          ...
          '<span class="text-[var(--endspace-text-primary)] font-bold group-hover:text-black">' +
          escapeHtml(date) +
          '</span>' +
          (category
            ? '<span class="w-px h-3 bg-[var(--endspace-border-base)]"></span><span class="tracking-wider">' +
              escapeHtml(category) +
              '</span>'
            : '') +
          '</div>' +
          '<h2 class="text-2xl md:text-3xl font-black text-[var(--endspace-text-primary)] mb-4 leading-tight group-hover:text-black">' +
          highlight(p.title, term) +
          '</h2>' +
          '<p class="text-[var(--endspace-text-secondary)] text-sm leading-relaxed line-clamp-3 mb-6 font-medium group-hover:text-black/70">' +
          highlight(summary, term) +
          '</p>' +
          '<div class="mt-auto flex items-center justify-between">' +
          '<div class="flex gap-2">' +
          tagsHtml +
          '</div>' +
          '<div class="flex items-center gap-2 text-[var(--endspace-text-primary)] text-xs font-bold uppercase tracking-wider group-hover:text-black"><span>' +
          ((window.__aicI18n && window.__aicI18n.access) || 'Access') +
          '</span></div>' +
          '</div>' +
          '</div>' +
          '</div>' +
  ...
  function initSearch() {
    ...
    if (initial) {
      input.value = initial;
      clear && clear.classList.remove('hidden');
      doSearch(wrapper, emptyEl, initial);
    }
    ...
    input.addEventListener('input', function () {
      ...
      debounceId = setTimeout(function () {
        doSearch(wrapper, emptyEl, input.value);
      }, 200);
    });
    if (clear)
